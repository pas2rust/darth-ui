...
pub struct Err {
    message: &'static str,
    class: &'static str,
    seconds_to_check_regex: Option<f32>,
    pattern: &'static str,
}
#[derive(DarthRust)]
pub struct Props {
...
#[component]
pub fn Text(props: Props) -> impl IntoView {
    let (read_input, write_input) = create_signal(props.value.to_string());

    view! {
        <label class=props.class for=props.label.forhtml name=props.name id=props.label.id>
            {props.label.name}:
        </label>
        <div class=props.container.class>
