...
pub fn case(props: &Toast, toaster: &Element, toast: &Element) -> String {
    ...
    match props.position {
        ToastPosition::TopStart => {
            class_output =
                "transition: 3s ease-in-out; transform: translateY(-100%); opacity:0;".to_string();
            ...
            toast.set_class_name(
                "animate-slideLeftToRight mx-auto m-4 p-2 rounded-lg pointer-events-auto overflow-hidden",
            );
            add_class_to_toast();
        }
        ToastPosition::TopMid => {
            ...
            toast.set_class_name(
                "animate-slideTopToBottom mx-auto m-4 p-2 rounded-lg pointer-events-auto overflow-hidden",
            );
            add_class_to_toast();
        }
        ToastPosition::TopEnd => {
            class_output =
                "transition: 3s ease-in-out; transform: translateX(100%); opacity:0;".to_string();
            toaster.set_class_name(
                "top-0 right-0 fixed z-50 flex flex-col-reverse gap-2 justify-end transition pointer-events-none p-4",
            );
            toast.set_class_name(
                "animate-slideRightToLeft mx-auto m-4 p-2 rounded-lg pointer-events-auto overflow-hidden max-w-md",
            );
            add_class_to_toast();
        }
        ToastPosition::BottomStart => {
            ...
            toast.set_class_name(
                "animate-toastinmx-auto m-4 p-2 rounded-lg pointer-events-auto overflow-hidden",
            );
            add_class_to_toast();
        }
